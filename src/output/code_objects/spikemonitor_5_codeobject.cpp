...
void _run_spikemonitor_5_codeobject()
{
...
const size_t _num_source_idx = 100;
    ...
    int32_t* __restrict  _ptr_array_neurongroup_5_i = _array_neurongroup_5_i;
    int32_t* __restrict  _ptr_array_spikemonitor_5__source_idx = _array_spikemonitor_5__source_idx;


    //// MAIN CODE ////////////

    int32_t _num_events = _ptr_array_neurongroup_5__spikespace[_num_spikespace-1];
    ...
    if (_num_events > 0)
    {
        ...
        {
            const int _idx = _ptr_array_neurongroup_5__spikespace[_j];
            if (_idx >= 0) {
                _start_idx = _j;
                break;
            }
        ...
        if (_num_events > 0) {
            ...
            for(size_t _j=_start_idx; _j<_end_idx; _j++)
            {
                ...
                const int32_t _source_i = _ptr_array_neurongroup_5_i[_idx];
                const int32_t _to_record_i = _source_i;
                const double _to_record_t = _source_t;

                _dynamic_array_spikemonitor_5_i.push_back(_to_record_i);
                _dynamic_array_spikemonitor_5_t.push_back(_to_record_t);
                _ptr_array_spikemonitor_5_count[_idx-0]++;
            }
            _ptr_array_spikemonitor_5_N[0] += _num_events;
        }
    }
...
const size_t _num_source_idx = 100;
    ...
    std::cout << "Number of spikes: " << _ptr_array_spikemonitor_5_N[0] << endl;
}
